...
function populateCategories(){ const s=$("category"); if(!s)return; placeholder(s,"Select vehicle category"); state.categories.forEach(c=>{const o=document.createElement("option");o.value=c.name;o.textContent=c.name;o.dataset.id=c.id;s.appendChild(o);}); }
...
async function initialize(){
    ...
    try{
        const [h,c,v]=await Promise.all([request(API.health),request(API.categories),request(API.vehicles)]);
        state.categories=Array.isArray(c?.categories)?c.categories:[]; state.vehicles=Array.isArray(v?.vehicles)?v.vehicles:[];
        populateCategories();updateForm();
